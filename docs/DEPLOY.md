# DEPLOY — standing this up on Windows or Linux

`install.py` is cross-platform (Windows + Linux), detects your hardware, and
picks a matching llama.cpp release automatically. This doc covers the manual
path, flags, and re-tuning — for the common case just run the installer.
...
```
install.py                    # ONE-SHOT installer + validator (Windows + Linux)
...
```
llama.cpp/          prebuilt llama.cpp release matching your OS + GPU backend
models/*.gguf        the model weights
run.ps1 / run.sh     restart script for whatever you last installed
