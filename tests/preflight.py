#!/usr/bin/env python3
"""Benchmark preflight: verify no stray llama processes and a clean GPU
baseline before starting a measured run.

Report-only by default; --kill terminates stray llama/bench/install processes
(the historical results in docs/RESULTS.md were only reproducible after doing
exactly that). Exits 0 when clean, 1 when anything is still in the way, so
...
def main():
    ...
    ap.add_argument("--kill", action="store_true",
                    help="terminate stray llama/bench/install processes")
