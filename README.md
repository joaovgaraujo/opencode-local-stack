# Local coding-agent stack — llama.cpp + OpenCode (Windows & Linux)

Run a local LLM fully on your own machine, sized to whatever GPU/RAM you
actually have, with the coding agent **OpenCode** as the frontend. Pick from
six current small-to-mid models (35B-A3B down to 4B) across a range of
quantizations; a GUI installer detects your hardware and only shows you the
combinations that plausibly fit.
...
```
git clone <this-repo-url> local-coding-agent
cd local-coding-agent
python install.py
```

That's it. `install.py` is cross-platform (Windows + Linux), stdlib-only (no
`pip install` needed), and:

1. **Detects your hardware** — OS, GPU vendor + VRAM, RAM, free disk.
2. **Opens a picker** (GUI if Tkinter is available, else a text wizard) showing
   every model/quantization combination, sorted by whether it *fits*, is a
   *tight fit*, or *won't fit* on your machine.
3. **Downloads** a matching prebuilt llama.cpp release + the GGUF you picked
   (or reuses ones you already have).
4. **Starts the server**, waits for `/health`, runs the validation suite
   (short completion, code-gen, tool-calling, 30k-token needle-in-haystack),
   and logs peak VRAM/RAM.
...
```
install.py                    one-shot installer + validator (Windows + Linux, GUI + CLI)
...
models/                       drop a GGUF here, or let the installer download it
...
- Models: [unsloth](https://huggingface.co/unsloth) GGUF quantizations of
